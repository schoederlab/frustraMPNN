...
class MegaScaleDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, split):
        ...
        df = pd.read_csv(fname, usecols=["frustration", "mut_type", "WT_name", "aa_seq", "wt_seq"]) # removed column "dG_ML", add "wt_seq"
        ...
        for wt_name in tqdm(self.wt_names):
            # print(wt_name)
            wt_rows = df.query('WT_name == @wt_name').reset_index(drop=True) # remove  mut_type == "wt" in query
            # print(wt_rows)
    ...
    def __getitem__(self, index):
        ...
        for i, row in mut_data.iterrows():
            ...
            ddG = torch.tensor([float(row.frustration)], dtype=torch.float32) # removed (-) sign
...
class FireProtDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, split):
        ...
        df = pd.read_csv(filename).dropna(subset=['frustration'])
        ...
        # print(f"Split {self.split} has {len(self.split_wt_names[self.split])} proteins")

        self.reweighting = cfg.training.reweighting
        ...
        for wt_name in self.wt_names:
            ...
            if split == 'test':
                ...
                # print(f"wt_seq: {self.mut_rows[wt_name]}")
        ...
        # print(f"Loaded {len(self.wt_names)} proteins for split {self.split}")
        # print(f"Weights: {self.weight_method}")
        # print(f"Rows: {self.weight_rows}")
    ...
    def __getitem__(self, index):
        ...
        for i, row in data.iterrows():
            ...
            frustration = None if row.frustration is None or isnan(row.frustration) else torch.tensor([row.frustration], dtype=torch.float32)
            if self.reweighting:
                weight = row[self.weight_method]
                # print(f"Weight: {weight}")
                # print(f"Row: {row.mutant_type}")
                mut = Mutation(pdb_idx, pdb[0]['seq'][pdb_idx], row.mutation, frustration, wt_name, weight)
            else:
                mut = Mutation(pdb_idx, pdb[0]['seq'][pdb_idx], row.mutation, frustration, wt_name, 1)
