...
def get_residue_locations(pdbfile, chains=""):
    ...
    surface_residues = select_surface_residues(pdbfile, surface=1, boundary=0, core=0, chains=chains)
    boundary_residues = select_surface_residues(pdbfile, surface=0, boundary=1, core=0, chains=chains)
    ...
    surface_dict = {res: 'surface' for res in surface_residues.split(',')} if surface_residues else {}
    boundary_dict = {res: 'boundary' for res in boundary_residues.split(',')} if boundary_residues else {}
    core_dict = {res: 'core' for res in core_residues.split(',')} if core_residues else {}
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate plots comparing errors and correlations across protein locations.')
    ...
    parser.add_argument('--output', '-o', default='location_analysis_plot.png', help='Output file name (default: location_analysis_plot.png)')
    parser.add_argument('--data-output', default=None, help='Path to save the error and correlation data as CSV')
    ...
    parser.add_argument('--chains', default="", help='Comma-separated list of chains to analyze (default: all chains)')
    ...
    init("-corrections::beta_nov16 -ignore_unrecognized_res true -mute all -out:level 0", silent=True)
    ...
    error_per_location = {
        loc: {'MSE': 0, 'RMSE': 0} for loc in location_categories
    }

    correlation_per_location = {
        loc: {r'R^2': 0, 'Spearman': 0} for loc in location_categories
    }
    ...
    for loc in location_categories:
        ...
        r2 = np.corrcoef(df_loc['furstration_calc'].tolist(), df_loc['frustration_pred'].tolist())[0, 1] ** 2
        spearman = df_loc[['furstration_calc', 'frustration_pred']].corr(method='spearman').iloc[0, 1]
    ...
    if args.data_output:
        ...
        r2_all = np.corrcoef(df['furstration_calc'].tolist(), df['frustration_pred'].tolist())[0, 1] ** 2
        spearman_all = df[['furstration_calc', 'frustration_pred']].corr(method='spearman').iloc[0, 1]
    ...
    df_counts = unique_residues['location'].value_counts().reindex(['surface', 'boundary', 'core', 'other']).fillna(0)
    df_counts.index = [loc.capitalize() for loc in df_counts.index]
    
    # Use grey color for the count bars
    bars_count = sns.barplot(x=df_counts.index, y=df_counts.values, ax=ax[0], color='darkgrey')
    ...
    bars_error = sns.barplot(data=df_loc_melted, x='location', y='Value', hue='error_type', 
                palette=palette_error, ax=ax[1], alpha=0.9)
    ...
    ax[1].set_xticks(range(len(df_loc_melted['location'].unique())), 
    ...
    bars_loc = sns.barplot(data=df_corr_loc_melted, x='location', y='Value', hue='corr_type', 
                       ax=ax[2], alpha=0.9, palette=palette_corr)
    ...
    ax[2].set_xticks(range(len(df_corr_loc_melted['location'].unique())), 
