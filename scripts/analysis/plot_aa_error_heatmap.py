...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate heatmap of error metrics per amino acid mutation."
    )
    ...
    parser.add_argument(
        "--output",
        "-o",
        default="aa_error_heatmap.png",
        help="Output file name (default: aa_error_heatmap.png)",
    )
    parser.add_argument(
        "--data-output", default=None, help="Path to save the error matrix data as CSV"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--log_scale", action="store_true", help="Use logarithmic scale for the heatmap"
    )
    parser.add_argument(
    ...
    cbar.set_label(
        f"{metric_name} {('(Log Scale)' if args.log_scale else '')}",
        fontsize=12,
        fontweight="normal",
        rotation=270,
        labelpad=20,
