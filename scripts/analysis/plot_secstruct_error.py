...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate plots comparing errors and correlations across secondary structure elements.')
    ...
    parser.add_argument('--output', '-o', default='error_correlation_plot.png', help='Output file name (default: error_correlation_plot.png)')
    ...
    chains_secstruct = {
        chain: calc_secstructure(pose, chain) for chain in get_chains(pose)
    }
    ...
    for ss in df['ss'].unique():
        ...
        r2 = np.corrcoef(df_ss['furstration_calc'].tolist(), df_ss['frustration_pred'].tolist())[0, 1] ** 2
        spearman = df_ss[['furstration_calc', 'frustration_pred']].corr(method='spearman').iloc[0, 1]
    ...
    bars_error = sns.barplot(data=df_melted, x='secstruct', y='Value', hue='error_type', 
    ...
    ax[1].set_xticks(range(len(df_melted['secstruct'].unique())), labels=['Helix', 'Sheet', 'Loops'])
    ax[1].set_xlabel('')
    ax[1].set_ylabel('')
    ...
    bars = sns.barplot(data=df_corr_melted, x='secstruct', y='Value', hue='corr_type', 
    ...
    ax[2].set_xticks(range(len(df_corr_melted['secstruct'].unique())), labels=['Helix', 'Sheet', 'Loops'])
    ax[2].set_xlabel('')
    ax[2].set_ylabel('')
    ax[2].set_ylim(0, 1)
