...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate scatter plot comparing calculated and predicted frustration values."
    )
    ...
    parser.add_argument(
        ...
        help="Output file name (default: comparison_plot.png)",
    )
    parser.add_argument(
        "--data-output", default=None, help="Path to save the comparison data as CSV"
    )
    parser.add_argument(
        "--no-errors", action="store_true", help="Disable error metrics (RMSE, MSE) on the plot"
    )
    parser.add_argument(
        "--no-chain-hue", action="store_true", help="Disable coloring points by chain"
    )
    ...
    if args.data_output:
        ...
        stats_df = pd.DataFrame(
            {"metric": ["MSE", "RMSE", "MAE", "R^2"], "value": [mse, rmse, mae, r2]}
        )
    ...
    if args.no_chain_hue:
        sns.scatterplot(
            x="furstration_calc", y="frustration_pred", data=df, ax=ax, s=19, alpha=0.6, zorder=10
        )
    else:
        sns.scatterplot(
            x="furstration_calc",
