...
run_docker() {
    ...
    # Log full docker command for reproducibility
    local docker_cmd=""
    if [ "$use_gpu" = true ]; then
        docker_cmd="docker run --rm --gpus all $volumes \"$DOCKER_IMAGE\" $command"
    else
        docker_cmd="docker run --rm $volumes \"$DOCKER_IMAGE\" $command"
    fi
    log_command "$docker_cmd" "$description"
    ...
    if [ "$TEST_MODE" = true ]; then
        ...
        if [[ "$command" == python* ]]; then
            ...
            if [ -n "$script_name" ]; then
                ...
                if [ -f "$(pwd)/scripts/$base_script" ]; then
                    ...
                    test_command "docker run --rm $volumes $DOCKER_IMAGE python -Wignore $(pwd)/scripts/$base_script --help" "$description"
                else
        ...
        # For built-in commands (like singleresidue or inference) that use the entrypoint
            ...
            test_command "docker run --rm $volumes $DOCKER_IMAGE singleresidue --help" "$description"
            ...
            test_command "docker run --rm $volumes $DOCKER_IMAGE inference --help" "$description"
            if [ $? -ne 0 ] && [ "$critical" = true ]; then
                log "ERROR" "Critical component test failed. Exiting now."
                exit 1
            fi
            ...
            # For other commands
            test_command "docker run --rm $volumes $DOCKER_IMAGE $first_cmd --help" "$description"
            if [ $? -ne 0 ] && [ "$critical" = true ]; then
                log "ERROR" "Critical component test failed. Exiting now."
                exit 1
            fi
    ...
    # Run the command
    if [ "$use_gpu" = true ]; then
        docker run --rm --gpus all $volumes "$DOCKER_IMAGE" $command > $temp_output 2>&1
    else
        docker run --rm $volumes "$DOCKER_IMAGE" $command > $temp_output 2>&1
    fi
...
show_help() {
    ...
    echo "  --docker-name IMAGE    Docker image name"
...
if [ -z "$DOCKER_IMAGE" ]; then
    log "ERROR" "Docker image name is required (--docker-name)"
    exit 1
fi
...
if ! docker image inspect "$DOCKER_IMAGE" > /dev/null 2>&1; then
    log "INFO" "Docker image not found locally. Attempting to pull..."
    if ! docker pull "$DOCKER_IMAGE" > /dev/null 2>&1; then
        log "ERROR" "Cannot find or pull Docker image '$DOCKER_IMAGE'"
        log "ERROR" "Please ensure the image name is correct and you have access to it"
        exit 1
    fi
    log "INFO" "Successfully pulled Docker image '$DOCKER_IMAGE'"
else
...
if check_file_exists "$CLEANED_PDB" "$REDO_CALC" "Cleaned PDB file"; then
    ...
    DOCKER_VOLUMES="-v ${PDB_DIR}:/app/pdbs -v ${OUT_DIR}:/app/results"
    DOCKER_CMD="singleresidue /app/pdbs/$(basename "$INPUT_PDB") --check_cb --clean --clean-only"
    
    run_docker "$DOCKER_VOLUMES" "$DOCKER_CMD" "PDB cleaning"
...
if [ "$SKIP_INFERENCE" = false ]; then
    if check_file_exists "$INFERENCE_OUTPUT" "$REDO_CALC" "Inference output file"; then
        ...
        DOCKER_VOLUMES="-v ${OUT_DIR}:/app/results -v ${WEIGHTS_DIR}:/app/inference/weights"
        DOCKER_CMD="inference --pdb /app/results/$(basename "$INPUT_PDB") \
            --model_path /app/inference/weights/$(basename "$WEIGHTS_PATH") \
            --output /app/results/${BASENAME}_inference_results.csv"
        
        run_docker "$DOCKER_VOLUMES" "$DOCKER_CMD" "Inference" true true "INFERENCE_TIME_MS" true
    fi
else
...
if [ "$SKIP_SINGLERESIDUE" = false ]; then
    if check_file_exists "$FRUSTRATION_PKL" "$REDO_CALC" "Singleresidue frustration file"; then
        ...
        DOCKER_VOLUMES="-v ${OUT_DIR}:/app/results"
        DOCKER_CMD="singleresidue /app/results/$(basename "$INPUT_PDB") \
            -o /app/results/${BASENAME}_singleresidue_frustration.pkl"
        
        run_docker "$DOCKER_VOLUMES" "$DOCKER_CMD" "Singleresidue frustration calculation" false true "SINGLERESIDUE_TIME_MS" true
    fi
else
...
if [ "$SKIP_DATA_ANALYSIS" = false ]; then
    ...
    if check_file_exists "$PLOT_OUTPUT" "$REDO_PLOT" "Comparison plot"; then
        ...
        DOCKER_VOLUMES="-v ${OUT_DIR}:/app/results -v $(pwd)/scripts:/app/scripts"
        ...
        DOCKER_CMD="python -Wignore /app/scripts/plot_comparison.py \
        ...
        run_docker "$DOCKER_VOLUMES" "$DOCKER_CMD" "Frustration comparison plot generation"
    fi
    ...
    if check_file_exists "$PLOT_OUTPUT" "$REDO_PLOT" "AA error heatmap"; then
        ...
        DOCKER_CMD="python -Wignore /app/scripts/plot_aa_error_heatmap.py \
            /app/results/${BASENAME}_singleresidue_frustration.pkl \
            /app/results/${BASENAME}_inference_results.csv \
        ...
        run_docker "$DOCKER_VOLUMES" "$DOCKER_CMD" "Amino acid error heatmap generation"
    fi
    ...
    if check_file_exists "$PLOT_OUTPUT" "$REDO_PLOT" "Secondary structure analysis plot"; then
        ...
        DOCKER_CMD="python -Wignore /app/scripts/plot_secstruct_error.py \
            /app/results/${BASENAME}_singleresidue_frustration.pkl \
            /app/results/${BASENAME}_inference_results.csv \
        ...
        run_docker "$DOCKER_VOLUMES" "$DOCKER_CMD" "Secondary structure analysis plot generation"
    fi
    ...
    if check_file_exists "$PLOT_OUTPUT" "$REDO_PLOT" "Location analysis plot"; then
        ...
        DOCKER_CMD="python -Wignore /app/scripts/plot_location_error.py \
            /app/results/${BASENAME}_singleresidue_frustration.pkl \
            /app/results/${BASENAME}_inference_results.csv \
            /app/results/$(basename "$INPUT_PDB") \
        ...
        run_docker "$DOCKER_VOLUMES" "$DOCKER_CMD" "Location analysis plot generation"
    fi
else
